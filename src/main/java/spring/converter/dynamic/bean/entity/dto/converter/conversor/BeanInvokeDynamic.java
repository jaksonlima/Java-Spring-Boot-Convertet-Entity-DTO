...
public class BeanInvokeDynamic {
    ...
    public static <T> T getFieldValue(Object javaBean, String fieldName) {
        try {
            var fieldValue = getCachedFunction(javaBean.getClass(), fieldName).apply(javaBean);
    ...
    private static Function getCachedFunction(Class<?> javaBeanClass, String fieldName) {
        var function = CACHE.get(javaBeanClass).get(fieldName);
        if (function != null) {
            return function;
        }

        var createAndCacheFunction = createAndCacheFunction(javaBeanClass, fieldName);
    ...
    private static Function cacheAndGetFunction(String path, Class<?> javaBeanClass, Function functionToBeCached) {
        var cachedFunction = CACHE.get(javaBeanClass).putIfAbsent(path, functionToBeCached);
